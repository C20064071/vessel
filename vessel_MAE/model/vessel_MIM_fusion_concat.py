...
class vessel_MIM(nn.Module):
    def __init__(self, img_size=(48,512), patch_size=(16,32), mask_ratio=0.5, in_chans=1,
    ...
    def forward_decoder(self, x, mask_ratio):
        ...
        return x, mask, ids_restore, feature_decoder
    ...
    def forward_fom_loss(self,cls_encoder,cls_decoder,fom):
        """
        feature: [N, embed_dim]
        fom: [N, 1]
        """
        feature = torch.cat([cls_encoder,cls_decoder],dim=1)
    ...
    def forward(self, imgs, fom):
        latent, cls_encoder = self.forward_encoder(imgs)
        pred, mask, ids_restore,cls_decoder = self.forward_decoder(latent, self.mask_ratio)  # [N, L, p*p*3]
        mae_loss = self.forward_mae_loss(imgs, pred, mask)
        fom_loss = self.forward_fom_loss(cls_encoder,cls_decoder,fom)
